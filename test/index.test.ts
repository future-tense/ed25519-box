import test from 'ava'

import { eddsa as Eddsa } from 'elliptic';
import randombytes from 'randombytes';

import { encrypt, decrypt } from '../src'

const ec = new Eddsa('ed25519');

function generatePubKey(seed: Buffer): Buffer {
    const key = ec.keyFromSecret(seed);
    return Buffer.from(key.getPublic())
}

const aliceSeed = Buffer.from(randombytes(32));
const bobSeed = Buffer.from(randombytes(32));
const alicePubkey = generatePubKey(aliceSeed);
const bobPubkey = generatePubKey(bobSeed);

test('encrypt + decrypt', async (t) => {
    const data = Buffer.from('', 'hex');
    const cipher = Buffer.from(await encrypt(aliceSeed, bobPubkey, data, null, false));
    const result = Buffer.from(await decrypt(bobSeed, alicePubkey, cipher, null, false));
    t.true(result.compare(data) === 0)
});
